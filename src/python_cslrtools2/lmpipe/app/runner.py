...
class CliAppRunner[K: str](LMPipeRunner[K]):
    ...
    def __init__(
        ...
        self.search_progress_task_ids: dict[int, TaskID] = {}
        ...
        self.batch_progress_task_ids: dict[int, TaskID] = {}
        ...
        self.frames_progress_task_ids: dict[int, TaskID] = {}
    ...
    def on_start_batch_job(self, runspec: RunSpec[Path]):
        ...
        self.search_progress_task_ids[id(self)] = self.rich_client.call_method(
            self.search_progress_ref,
            Progress.add_task,
        ...
        self.batch_progress_task_ids[id(self)] = self.rich_client.call_method(
            self.batch_progress_ref,
            Progress.add_task,
            DESC_TEMPLATE.format(
                deco="bold blue",
    ...
    def on_submit_batch_task(self, runspec: RunSpec[Path], task_id: int):
        ...
        search_progress_task_id = self.search_progress_task_ids.get(id(self))
    ...
    def on_determined_batch_task_count(self, runspec: RunSpec[Path], task_count: int):
        ...
        batch_progress_task_id = self.batch_progress_task_ids.get(id(self))
    ...
    def on_failure_batch_task(self, runspec: RunSpec[Path], task_id: int, error: Exception):
        ...
        batch_progress_task_id = self.batch_progress_task_ids.get(id(self))
        if batch_progress_task_id is None:
            return
    ...
    def on_end_batch_job(self, runspec: RunSpec[Path]):
        ...
        search_progress_task_id = self.search_progress_task_ids.pop(id(self), None)
        if search_progress_task_id is not None:
            self.rich_client.call_method(
                self.search_progress_ref,
                Progress.stop_task,
        ...
        batch_progress_task_id = self.batch_progress_task_ids.pop(id(self), None)
        if batch_progress_task_id is not None:
            self.rich_client.call_method(
                self.batch_progress_ref,
                Progress.stop_task,
    ...
    def on_start_frames_job(self):
        ...
        self.frames_progress_task_ids[id(self)] = self.rich_client.call_method(
            self.frames_progress_ref,
            Progress.add_task,
            DESC_TEMPLATE.format(
                deco="bold blue",
    ...
    def on_determined_frames_task_count(self, task_count: int):
        ...
        frames_progress_task_id = self.frames_progress_task_ids.get(id(self))
    ...
    def on_failure_frames_task(self, task_id: int, error: Exception):
        ...
        frames_progress_task_id = self.frames_progress_task_ids.get(id(self))
        if frames_progress_task_id is None:
            return
    ...
    def on_end_frames_job(self):
        ...
        frames_progress_task_id = self.frames_progress_task_ids.pop(id(self), None)
        if frames_progress_task_id is not None:
            self.rich_client.call_method(
                self.frames_progress_ref,
                Progress.stop_task,
