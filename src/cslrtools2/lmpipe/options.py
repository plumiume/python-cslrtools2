...
logging, runtimeresources, executor, collector, lmpipe options used
throughout the pipeline processing.

Attributes:
    DEFAULT_LOGGING_OPTIONS (LoggingOptions): Default Logging
        configuration for pipeline execution..
    DEFAULT_RUNTIME_RESOURCES (RuntimeResources): Default Runtime
        resource allocation for parallel execution..
    DEFAULT_EXECUTOR_OPTIONS (ExecutorOptions): Default Parallel
        execution configuration..
    DEFAULT_COLLECTOR_OPTIONS (CollectorOptions): Default Output
        collection and saving configuration..
    DEFAULT_LMPIPE_OPTIONS (LMPipeOptions): Combined default lmpipe
        configuration.
"""
...
DEFAULT_LOGGING_OPTIONS: LoggingOptions = {"log_level": "info", "log_file": None}
...
class ExecutorOptions(TypedDict):
    ...
    """Execution mode: 'loky' for robust multiprocessing, 'thread' for
    I/O bound tasks, or None for auto-detection."""
    executor_type: ExecutorType
    """Executor backend: 'process' for CPU-bound tasks, 'thread' for
    I/O-bound, or None for auto-selection."""
...
class CollectorOptions(TypedDict):
    ...
    """File format for landmark coordinates. Supported: '.csv', '.json',
    '.npy', '.npz', '.pt', '.safetensors', '.zarr'. None = disabled."""
    landmark_matrix_save_exist_rule: ExistRule
    """Behavior when output file exists: 'skip' (keep existing), 'overwrite' (replace),
    ...
    """Framework for video encoding: 'cv2' (OpenCV), 'matplotlib', or None
    for auto-detection."""
    ...
    """Framework for live preview display: 'cv2' (OpenCV window), 'pil'
    (PIL), 'matplotlib', 'torchvision', or None to disable."""
...
class LMPipeOptionsGroup(
    ...
    """CLI argument group combining logging and runtimeresources and
    executor and collector options."""
...
def logging_options_group_to_dict(group: LoggingOptionsGroup.T) -> LoggingOptions:
    ...
    return {"log_level": group.log_level, "log_file": group.log_file}
