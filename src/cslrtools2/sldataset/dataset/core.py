...
from typing import TYPE_CHECKING, Any, Iterable, Iterator, Mapping, Self, Sequence
...
type DefaultSLDataset[Kvid: str, Klm: str, Ktgt: str] = Dataset[
    DefaultSLDatasetItem[Kvid, Klm, Ktgt]
]

type DefaultIterableSLDataset[Kvid: str, Klm: str, Ktgt: str] = IterableDataset[
    DefaultSLDatasetItem[Kvid, Klm, Ktgt]
]
...
class SLDatasetBatch[Kmeta: str, Kvid: str, Klm: str, Ktgt: str](
    ...
    def to(self, device: torch.device) -> Self:
        return type(self)(
            dataset=self.dataset.to_partially(device), item=self.item.to(device)
        )
    ...
    def from_batch(
        ...
        return cls(
            dataset=SLDataset(
                ...
                connections={k: as_tensor(v) for k, v in dataset.connections.items()},
                items=dataset.items,
            ),
            item=SLDatasetItem(
                videos={
                    kvid: torch.cat([as_tensor(item.videos[kvid]) for item in batch])
                    for kvid in b0.videos.keys()
                },
                landmarks={
                    klm: torch.cat([as_tensor(item.landmarks[klm]) for item in batch])
                    for klm in b0.landmarks.keys()
                },
                targets={
                    ktgt: torch.cat([as_tensor(item.targets[ktgt]) for item in batch])
...
class SLDataset[Kmeta: str, Kvid: str, Klm: str, Ktgt: str, Vconn: ArrayLike](
    SLKeyHolder[Kmeta, Kvid, Klm, Ktgt], Dataset[DefaultSLDatasetItem[Kvid, Klm, Ktgt]]
):
    def __init__(
        self,
        metadata: Mapping[Kmeta, Any],
        connections: Mapping[tuple[Klm, Klm], Vconn],
        items: DefaultSLDataset[Kvid, Klm, Ktgt]
        | Sequence[DefaultSLDatasetItem[Kvid, Klm, Ktgt]],
    ):
    ...
    ) -> SLDataset[Kmeta, Kvid, Klm, Ktgt, Tensor]:
        return SLDataset(
            metadata=self.metadata,
            connections={
                k: as_tensor(v).to(device) for k, v in self.connections.items()
            },
            items=self.items,
        )
    ...
    def from_zarr(
        ...
        return SLDataset(metadata=metadata, connections=connections, items=items)
...
class IterableSLDataset[Kmeta: str, Kvid: str, Klm: str, Ktgt: str, Vconn: ArrayLike](
    ...
    def __init__(
        ...
        | Iterable[DefaultSLDatasetItem[Kvid, Klm, Ktgt]],
    ):
        self.metadata = metadata
        self.connections = connections
        self.items = items
    ...
    def to_partially(
        self, device: torch.device
    ) -> IterableSLDataset[Kmeta, Kvid, Klm, Ktgt, Tensor]:
...
def dataset_to_zarr(
    dataset: (
        SLDataset[Any, Any, Any, Any, Any] | IterableSLDataset[Any, Any, Any, Any, Any]
    ),
    ...
    for (klm1, klm2), conn in dataset.connections.items():
        connections_group.create_array(f"{klm1}.{klm2}", data=np.asarray(conn))

    items_group = root_group.create_group("items")
    item_iter = iter(
        dataset
        if isinstance(dataset, IterableSLDataset)
