...
class TransformSLDataset[Kmeta: str, Kvid: str, Klm: str, Ktgt: str, Vconn: ArrayLike](
    ...
    def __getitem__(self, index: int) -> TensorSLDatasetItem[Kvid, Klm, Ktgt]:
        item = self.dataset[index]

        tensor_item = SLDatasetItem(
            videos={kvid: as_tensor(v) for kvid, v in item.videos.items()},
            landmarks={klm: as_tensor(v) for klm, v in item.landmarks.items()},
            targets={ktgt: as_tensor(v) for ktgt, v in item.targets.items()},
        )
