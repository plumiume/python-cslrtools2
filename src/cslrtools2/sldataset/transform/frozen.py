...
BoundaryConditionTuple = tuple[
    BoundaryConditionLiteral, BoundaryConditionLiteral
] | tuple[
    Literal[1, 2], ArrayLike
]
BoundaryConditionType = BoundaryConditionLiteral | BoundaryConditionTuple

ExtrapolationType = Literal['periodic'] | bool | None

class CubicSplineInterpolateMissingLandmarks[
    ...
    def __call__(
        ...
        for klm in self.landmark_keys:
            ...
            landmarks[klm] = torch.tensor(
                np.stack(store).reshape(n, t, v, c),
                dtype=vlm.dtype,
                device=vlm.device
            )
