...
@pytest.fixture
def sample_single_key_result(sample_landmark_matrix: NDArrayFloat) -> dict[Literal["pose"], NDArrayFloat]:
    """Single key result mapping."""
    return {"pose": sample_landmark_matrix}


@pytest.fixture
def sample_multi_key_result(sample_landmark_matrix: NDArrayFloat) -> dict[str, NDArrayFloat]:
...
class TestCSVLMSCFileOperations:
    ...
    def test_save_single_key_csv(
        ...
        with csv_file.open("r", encoding="utf-8") as f:
            ...
            # Each row should have key, sample_index, and 99 values (33*3)
            assert all("key" in row for row in rows)
            assert all("sample_index" in row for row in rows)
            assert all(len(row) == 2 + 99 for row in rows)  # key + sample_index + 99 values            # Check first row structure
            first_row = rows[0]
            assert "key" in first_row
            assert "sample_index" in first_row
            assert first_row["key"] == "pose"
            assert first_row["sample_index"] == "0"
...
class TestCSVLMSCErrorHandling:
    ...
    def test_inconsistent_sample_width_raises_error(
        ...
        with pytest.raises(ValueError, match="Inconsistent landmark sample width"):
            collector._open_file(temp_output_dir)
            try:
                collector._append_result(first_result)
                collector._append_result(second_result)
            finally:
                collector._close_file()
    
    def test_writer_not_prepared_raises_error(self):
        """Test accessing writer before opening file raises error."""
        collector = CsvLandmarkMatrixSaveCollector[str]()
        
        # Directly calling _ensure_writer without _open_file
        with pytest.raises(RuntimeError, match="CSV landmark directory not prepared"):
