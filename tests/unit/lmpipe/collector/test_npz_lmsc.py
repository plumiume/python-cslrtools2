...
class TestNPZLMSCFileOperations:
    ...
    def test_save_single_key_npz(
        ...
        with np.load(npz_file, allow_pickle=True) as data:
            assert "pose" in data
            assert data["pose"].shape == (
                1,
                10,
                33,
                3,
            )  # (num_appends, frames, landmarks, coords)
...
class TestNPZLMSCEdgeCases:
    ...
    def test_nested_output_directory_creation(self, temp_output_dir: Path):
        ...
        nested_path.mkdir(
            parents=True
        )  # NPZ doesn't use landmarks/ dir, so create parent
    ...
    def test_mixed_keys_across_appends(self, temp_output_dir: Path):
        ...
        collector = NpzLandmarkMatrixSaveCollector[
            Literal["pose", "left_hand", "right_hand"]
        ]()
...
class TestNPZLMSCEmptyBuffer:
    """Tests for empty buffer edge cases."""

    def test_close_file_with_empty_buffer(self, temp_output_dir: Path):
        """Test _close_file creates empty npz when buffer is empty."""
        ...
        # Verify empty npz file was created
        npz_file = temp_output_dir / "landmarks.npz"
        assert npz_file.exists()

        # Verify it's a valid empty npz
        with np.load(npz_file, allow_pickle=True) as data:
            assert len(data.files) == 0
