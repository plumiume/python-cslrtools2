...
class TestRunSpecDirectCreation:
    ...
    def test_create_with_path_source(
        self, temp_video_file: Path, temp_output_dir: Path
    ):
...
class TestRunSpecFromPathLikes:
    ...
    def test_from_pathlikes_nonexistent_source_raises_error(self, tmp_path: Path):
...
class TestRunSpecEdgeCases:
    ...
    def test_dst_path_not_required_to_exist(
        self, temp_video_file: Path, tmp_path: Path
    ):
